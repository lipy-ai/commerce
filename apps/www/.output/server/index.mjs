...
import { UserCircle, ShoppingBag, Search } from 'lucide-react';
...
const se = defineHandlerCallback(async ({ request: t, router: r, responseHeaders: o }) => {
  if (typeof p$1.renderToReadableStream == "function") {
    const n = await p$1.renderToReadableStream(jsx(b, { router: r }), { signal: t.signal });
    isbot(t.headers.get("User-Agent")) && await n.allReady;
    const s = transformReadableStreamWithRouter(r, n);
    return new Response(s, { status: r.state.statusCode, headers: o });
  }
  if (typeof p$1.renderToPipeableStream == "function") {
    const n = new PassThrough();
    try {
      const i = p$1.renderToPipeableStream(jsx(b, { router: r }), { ...isbot(t.headers.get("User-Agent")) ? { onAllReady() {
        i.pipe(n);
      } } : { onShellReady() {
        i.pipe(n);
      } }, onError: (d, g) => {
        console.error("Error in renderToPipeableStream:", d, g);
      } });
    } catch (i) {
      console.error("Error in renderToPipeableStream:", i);
    }
    const s = transformPipeableStreamWithRouter(r, n);
    return new Response(s, { status: r.state.statusCode, headers: o });
  }
...
function ce() {
  var _a;
  const t = ae(), r = t.routes.__root__ = t.routes.__root__ || {};
  r.assets = r.assets || [];
  let o = "";
  const n = ie("client"), s = (_a = n.inputs[n.handler]) == null ? void 0 : _a.output.path;
  return s || j$1(s, "Could not find client entry in vinxi manifest"), r.assets.push({ tag: "script", attrs: { type: "module", suppressHydrationWarning: true, async: true }, children: `${o}import("${s}")` }), t;
}
function le() {
  const t = ce();
...
function N({ error: t }) {
  const r = useRouter(), o = useMatch({ strict: false, select: (n) => n.id === rootRouteId });
...
const de = ({ title: t, description: r, keywords: o, image: n }) => [{ title: t }, { name: "description", content: r }, { name: "keywords", content: o }, { name: "twitter:title", content: t }, { name: "twitter:description", content: r }, { name: "twitter:creator", content: "@tannerlinsley" }, { name: "twitter:site", content: "@tannerlinsley" }, { name: "og:type", content: "website" }, { name: "og:title", content: t }, { name: "og:description", content: r }, ...n ? [{ name: "twitter:image", content: n }, { name: "twitter:card", content: "summary_large_image" }, { name: "og:image", content: n }] : []], v = { default: "/logo/ico.svg" };
function l(...t) {
  return twMerge(clsx(t));
}
const k = G.forwardRef(({ className: t, type: r, suffixEl: o, prefixEl: n, size: s, ...i }, d) => jsxs("div", { className: l("border-input bg-background focus-within:border-ring focus-within:ring-ring flex items-center overflow-hidden rounded-md border focus-within:outline-none focus-within:ring-1", t), children: [n && jsx("span", { className: l("text-muted-foreground flex h-9 items-center px-4 [&>svg]:w-4", s === "lg" && "h-12"), children: n }), jsx("input", { type: r, className: l("placeholder:text-muted-foreground flex h-9 w-full bg-transparent py-1 text-sm shadow-sm outline-none transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:cursor-not-allowed disabled:opacity-50", !o && "pr-3", !n && "pl-3", s === "lg" && "h-12"), ref: d, ...i }), jsx("span", { className: "text-sm", children: o && jsx("span", { className: l("text-muted-foreground flex h-9 items-center px-2 [&>svg]:w-4", s === "lg" && "h-12"), children: o }) })] }));
k.displayName = "Input";
function R() {
...
const Ie = createStartHandler({ createRouter: Se, getRouterManifest: le })(se);

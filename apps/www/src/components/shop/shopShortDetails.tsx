...
export default function ShopShortDetails({ shopInfo }: any) {
	return (
		<div className="p-4">
			{/* Header with Shop Name and Rating */}
			<div className="flex items-center justify-between mb-4">
				<div className="flex-1">
					<div className="font-bold text-xl text-gray-800">{shopInfo.name}</div>
			...
			<div className="flex items-center mb-5">
				<div
					className={`w-2 h-2 rounded-full mr-2 ${shopInfo.isOpen ? "bg-green-500" : "bg-red-500"}`}
				/>
				<span
					className={`text-sm font-medium ${shopInfo.isOpen ? "text-green-600" : "text-red-600"}`}
				>
					{shopInfo.isOpen ? "Open Now" : "Closed"}
				</span>
				<div className="mx-2 h-4 border-r border-gray-300" />

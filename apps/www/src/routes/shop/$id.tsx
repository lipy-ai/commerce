...
if(isLoading) return <Loading/>
  ...
  return (
    <div key={categoryName} className="my-10">
      ...
      <div className="flex overflow-x-auto flex-nowrap  scrollbar-hide w-full" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }} >
        {productArray?.map((product: any, index) => (
          <ProductCard key={product.id} product={product} className={cn(index===0 ? "pl-4": index===productArray.length-1 ? "pr-4" : "", "flex-shrink-0 w-32 pl-4")} />
        ))}
      </div>
    </div>
  );
